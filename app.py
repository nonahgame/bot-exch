...
def ai_decision(df, stop_loss_percent=STOP_LOSS_PERCENT, take_profit_percent=TAKE_PROFIT_PERCENT, position=None, buy_price=None):
    ...
    if position == "long" and buy_price is not None:
        ...
        if close_price <= stop_loss:
            ...
            action = "sell"
        elif (kdj_j > kdj_d and kdj_j > 90.00 and macd_hist > 1.00 and ema1 > ema2 and rsi > 60.00):  # Existing KDJ/MACD sell condition
            logger.info(
    ...
    if action == "hold" and position is None:
        if (supertrend_trend == 0 and kdj_j < kdj_d and kdj_j < - 10.00):  # Modified: Buy on Supertrend Downtrend
            logger.info(
                f"Buy triggered by Supertrend: supertrend_trend=Down, close={close_price:.2f}"
            )
            action = "buy"
        elif (kdj_j < kdj_d and kdj_j < -5.00 and ema1 < ema2 and rsi < 20.00):  # Existing KDJ/MACD buy condition  and macd_hist < -0.01
            logger.info(
